...
//            guahao.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        GuahaoEntity guahaoEntity = guahaoService.selectOne(queryWrapper);
        if(guahaoEntity==null){
            if(guahao.getGuahaoStatusTypes() == 3){
...
//                            guahaoEntity.setYishengId(Integer.valueOf(data.get(0)));   //宠物医生 要改的
//                            guahaoEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            guahaoEntity.setGuahaoUuidNumber(data.get(0));                    //就诊识别码 要改的
//                            guahaoEntity.setGuahaoTime(sdf.parse(data.get(0)));          //挂号时间 要改的
    ...
    */
    ...
    public R add(@RequestBody GuahaoEntity guahao, HttpServletRequest request){
        ...
        if(guahaoEntity==null){
            ...
            YonghuEntity yonghuEntity = yonghuService.selectById(guahao.getYonghuId());
            YishengEntity yishengEntity = yishengService.selectById(guahao.getYishengId());
            if(yonghuEntity.getNewMoney() - yishengEntity.getYishengNewMoney() < 0){
                return R.error("余额不足请充值");
            }
            yonghuEntity.setNewMoney(yonghuEntity.getNewMoney() - yishengEntity.getYishengNewMoney());
            yonghuService.selectById(yonghuEntity);
