		...
		<div v-if="showFlag">
			<el-form :inline="true" :model="searchForm" class="form-content">
				<el-row :gutter="20" class="slt"
					...
					<el-form-item :label="contents.inputTitle == 1 ? '开始挂号时间' : ''">
						<el-date-picker value-format="yyyy-MM-dd" v-model="searchForm.guahaoTimeStartSearch" type="date"
							placeholder="开始时间">
						</el-date-picker>
					</el-form-item>
					...
					<el-form-item :label="contents.inputTitle == 1 ? '结束挂号时间' : ''">
						<el-date-picker value-format="yyyy-MM-dd" v-model="searchForm.guahaoTimeEndSearch" type="date"
							placeholder="结束时间">
						</el-date-picker>
					</el-form-item>
					...
					<el-form-item :label="contents.inputTitle == 1 ? '时间类型' : ''">
						<el-select v-model="searchForm.guahaoTypes" placeholder="请选择时间类型">
							...
							<el-option v-for="(item,index) in guahaoTypesSelectSearch" v-bind:key="index"
					...
					<el-form-item :label="contents.inputTitle == 1 ? '医生名称' : ''">
						<el-input prefix-icon="el-icon-search" v-model="searchForm.yishengName" placeholder="医生名称"
							clearable></el-input>
					</el-form-item>
					...
					<el-form-item :label="contents.inputTitle == 1 ? '科室' : ''">
						<el-select v-model="searchForm.yishengTypes" placeholder="请选择科室">
							<el-option label="=-请选择-=" value=""></el-option>
							<el-option v-for="(item,index) in yishengTypesSelectSearch" v-bind:key="index"
								:label="item.indexName" :value="item.codeIndex">
								<!--lable是要展示的名称-->
								<!--value是值-->
							</el-option>
				...
				<el-row class="ad"
					...
					<el-form-item>
						<el-button v-if="isAuth('guahao','新增')" type="success" icon="el-icon-plus"
							@click="addOrUpdateHandler()">新增</el-button>
						&nbsp;
						<el-button v-if="isAuth('guahao','删除')" :disabled="dataListSelections.length <= 0" type="danger"
							icon="el-icon-delete" @click="deleteHandler()">删除</el-button>
						&nbsp;
						<el-button v-if="isAuth('guahao','报表')" type="success" icon="el-icon-pie-chart"
							@click="chartDialog()">报表</el-button>
						&nbsp;
						<a style="text-decoration:none" class="el-button el-button--success"
							v-if="isAuth('guahao','导入导出')" icon="el-icon-download"
							href="http://localhost:8080/chongwuyiyuanyuyueguahao/upload/guahaoMuBan.xls">批量导入宠物挂号数据模板</a>
						&nbsp;
						<el-upload v-if="isAuth('guahao','导入导出')" style="display: inline-block"
							action="chongwuyiyuanyuyueguahao/file/upload" :on-success="guahaoUploadSuccess"
							:on-error="guahaoUploadError" :show-file-list=false>
							<el-button v-if="isAuth('guahao','导入导出')" type="success" icon="el-icon-upload2">批量导入宠物挂号数据
							</el-button>
						</el-upload>
						&nbsp;
						<!-- 导出excel -->
						<download-excel v-if="isAuth('guahao','导入导出')" style="display: inline-block"
							class="export-excel-wrapper" :data="dataList" :fields="json_fields" name="guahao.xls">
							<!-- 导出excel -->
			...
			<div class="table-content">
				<el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
					...
					v-if="isAuth('guahao','查看')" :data="dataList" v-loading="dataListLoading"
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yishengName"
						header-align="center" label="医生名称">
						<template slot-scope="scope">
							{{scope.row.yishengName}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yishengTypes"
						header-align="center" label="科室">
						<template slot-scope="scope">
							{{scope.row.yishengValue}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yishengPhoto"
						header-align="center" width="200" label="医生头像">
						<template slot-scope="scope">
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="yishengNewMoney" header-align="center" label="挂号价格">
						<template slot-scope="scope">
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="guahaoUuidNumber" header-align="center" label="就诊识别码">
						<template slot-scope="scope">
							{{scope.row.guahaoUuidNumber}}
						</template>
					</el-table-column>

					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="guahaoTime"
						header-align="center" label="挂号时间">
						<template slot-scope="scope">
							{{scope.row.guahaoTime}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="guahaoTypes"
						header-align="center" label="时间类型">
						<template slot-scope="scope">
							{{scope.row.guahaoValue}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="guahaoStatusTypes" header-align="center" label="挂号状态">
						<template slot-scope="scope">
					...
					<el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
						<template slot-scope="scope">
							<el-button v-if="isAuth('guahao','查看')" type="success" icon="el-icon-tickets" size="mini"
								@click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button v-if="isAuth('guahao','修改')" type="primary" icon="el-icon-edit" size="mini"
								@click="addOrUpdateHandler(scope.row.id)">修改</el-button>
							<el-button v-if="isAuth('guahao','状态') && scope.row.guahaoStatusTypes == 1" type="primary" icon="el-icon-edit" size="mini"
								@click="yijuzhen(scope.row.id)">已就诊</el-button>
							<el-button v-if="isAuth('guahao','取消挂号') && scope.row.guahaoStatusTypes == 1" type="primary" icon="el-icon-edit" size="mini"
								@click="tuikuan(scope.row.id,scope.row.yishengId)">退款</el-button>
									
							<el-button v-if="isAuth('guahao','删除')" type="danger" icon="el-icon-delete" size="mini"
